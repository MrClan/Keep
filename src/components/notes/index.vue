  ...
  <div>
    <div class="grid">
      <div class="grid-item" style="height:404px;width:100%;border-radius:57px; background-color: #2ecc71;color:#EEE;">
        <h3>Add New Note</h3>
        <form class="create-note" v-on:submit.prevent="createOrUpdateNote(currentNoteKey)" autocomplete="off" v-on:keyup.ctrl.enter="createOrUpdateNote(currentNoteKey)">
          ...
          <button type="submit" title="Save note">+</button>
        </form>
      ...
      <template v-for="(n,i) in notes">
        <div class="grid-item" v-bind:style="{backgroundColor: n.color}">
          ...
          <hr/>
          <p>
            <button @click="editNote(n.key)" title="Edit note" >
              <img src="static/edit.png"style="height:24px;opacity:0.75;">
            </button>
            <button title="Delete note" @click="deleteNote(n.key)">
              <img src="static/del.png" style="height:24px;opacity:0.75;">
            </button>
          </p>
        </div>
      </template>
  ...
  export default {
    ...
    methods: {
      editNote: function (key) {
        ...
        if (nk.length != 1) {
          console.error('More than one note with same key found. Action cancelled.')
          return
        }
      ...
      createOrUpdateNote: function (key) {
        if (this.title.trim() || this.content.trim()) {
          ...
          if (this.mode == modes.NEW) {
            let newId = firebaseDb.ref('notes').push({
              title: this.title,
              content: this.content
            }, (err) => {
              if (err) {
        ...
        notesRef.once('value', (snapshot) => {
          snapshot.forEach((child) => {
            notes.push({
              ...
              color: colors[parseInt(Math.random() * 21)],
              classes: ['grid-item', _this.heightClass[parseInt(Math.random() * 3)], _this.widthClass[
          ...
          notesRef.on('child_added', function (data) {
            ...
            if (note.length == 0) {
              ...
              if (!_this.isSelf) {
                _this.$toastr.info('A note was added from another device')
              }
              _this.isSelf = false
            }
          })
          ...
          notesRef.on('child_changed', function (data) {
            for (let i = 0; i < notes.length; i++) {
              if (notes[i].key == data.key) {
                ...
                debugger
                if (!_this.isSelf) {
                  _this.$toastr.info('A note was updated from another device')
                }
                _this.isSelf = false
                break
              }
          ...
          notesRef.on('child_removed', function (data) {
            ...
            for (let i = 0; i < notes.length; i++) {
              if (notes[i].key == data.key) {
                ...
                notes.splice(i, 1)
                if (!_this.isSelf) {
                  _this.$toastr.info('A note was deleted from another device')
                }
                _this.isSelf = false
                break
              }
